...
const ConversationModel = require('../../../models/chat/chatConnectionModel')

// const TopicsModel = require('../../../models/admin/master/academic/topicsModel');

/**
 * Remove New Assignments Dot in Assignments.
 * 
 * This Dot is mainly to know that there exists some activity regarding group by teacher.
 * This Dot is removed as student has taken inside of group and shown assignments.
 */
const updateStudentSeenStatus = (id, userId) => {
...
const getConversationId = (studentId, groupId) => {
      ...
      return new Promise(async (resolve, reject) => {
            ...
            try {
                  ...
                  ConversationModel.findOne({
                        ...
                        .exec()
                        .then(chatConnectionExists => {

                              console.log("chatConnectionExists");
                              console.log(chatConnectionExists);

                              if (chatConnectionExists) {
                                    resolve(chatConnectionExists._id);
                              } else {
                                    resolve("")
                              }
 ...
 */
exports.parseAssignmentData = (assignmentRecords, userId, userType, today) => {
      ...
      return new Promise(async (resolve, reject) => {
            ...
            for (let index = 0; index < assignmentRecords.length; index++) {
                  ...
                  let recordObj = {
                        ...
                        teacherId: record.groupId.teacherId._id,
                        teacherName: record.groupId.teacherId.surName ? record.groupId.teacherId.firstName + " " + record.groupId.teacherId.surName : record.groupId.teacherId.firstName,
                        ...
                        conversationId: await getConversationId(userId, record.groupId._id)
                  ...
                  if (!record.teacherDeleteAllStatus || record.teacherDeleteAllStatus == false) {
                        ...
                        if (userType == 1) { //teacher
                              ...
                              recordObj.deliveryTick = record.seenStudents.length >= record.activeStudentIds.length ? String(2) : String(1);
                        ...
                        if (record.sectionType == "HomeWork") {
                              ...
                              recordObj.projectWorkId = record.projectWork._id;
                              ...
                              if (userType == 2) { // For the student
                                    ...
                                    if ((record.projectWork.completedStudents && record.projectWork.completedStudents.length > 0) || (record.projectWork.notCompletedStudents && record.projectWork.notCompletedStudents.length > 0)) {
                                          ...
                                          if (record.projectWork.completedStudents && record.projectWork.completedStudents.length > 0) {

                                                let completed = record.projectWork.completedStudents.indexOf(userId);

                                                recordObj.response = (completed != -1) ? "true" : "";
                                                recordObj.reason = (completed != -1) ? "completed" : "";

                                          }

                                          if (recordObj.response == "") {

                                                if (record.projectWork.notCompletedStudents && record.projectWork.notCompletedStudents.length > 0) {

                                                      for (let index = 0; index < record.projectWork.notCompletedStudents.length; index++) {
                                                            const userId_ = record.projectWork.notCompletedStudents[index];
                                                      ...
                                                      recordObj.response = alreadyUpdated.userId ? "false" : "";
                                                      recordObj.reason = alreadyUpdated ? alreadyUpdated.reason ? alreadyUpdated.reason : "" : "";
